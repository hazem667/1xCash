...
async def sup_reason(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    from handlers.keyboards import support_admin_kb
...
def get_handler():
    ...
    async def sup_entry(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if update.message.text != label:
            return ConversationHandler.END
        return await sup_start(update, ctx)

    return ConversationHandler(
        entry_points=[MessageHandler(f.TEXT & ~f.COMMAND, sup_entry)],
        states={
            SUP_REASON: [
                MessageHandler(f.TEXT & ~f.COMMAND, sup_reason),
